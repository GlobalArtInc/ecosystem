...
#### `call<T extends Record<string, any>, K extends keyof T>(serviceName: string, methodName: string, payload?: Parameters<T[K]>[0]): Promise<UnwrapObservable<ReturnType<T[K]>>>`
