...
| `logRequests`     | `boolean`         | `false`     | Log every handled HTTP request                       |
| `fieldNaming`     | `FieldNaming`     | `'camelCase'` | Case of the log entry's own field names            |
| `includeHeaders`  | `string[]`        | `undefined` | Log only these headers; all of them when unset       |
| `logRequestBody`  | `boolean`         | `true`      | Include the request body in the log entry            |
| `logResponseBody` | `boolean`         | `false`     | Include the response body in the log entry           |
| `maxBodyLength`   | `number`          | `2048`      | Truncate a serialized body longer than this          |

### Field Naming

Log entries name their own fields in camelCase by default — `responseTime`, `statusCode`,
`remoteAddress`, `traceId`. Set `fieldNaming: "snake_case"` when the log pipeline expects snake_case
keys:
...
```json
{ "level": 30, "req": { "remote_address": "10.0.0.1" }, "res": { "status_code": 200 }, "response_time": 56 }
```

Only the entry's own fields are renamed. Everything the application or the client supplied —
`headers`, `metadata`, `query`, `params` and the request and response bodies — keeps its keys
verbatim, so a payload field named `accountId` stays `accountId`. Applies to the `json` and `pino`
formats; `text` output is unaffected.

### HTTP Payload Logging

By default every request header is written to the log entry, which makes lines large enough to be
dropped by log backends that enforce a per-line size limit, and leaks `cookie` and other
credential-carrying headers into log storage. Pass `includeHeaders` to keep only the headers you
actually query on, and cap payloads with `maxBodyLength`:

```typescript
LoggerModule.forRoot({
  format: "pino",
  logRequests: true,
  includeHeaders: ["user-agent", "x-request-id", "content-type"],
  logRequestBody: true,
...
Header matching is case-insensitive and applies to both request and response headers.
Bodies are sanitized with `sensitiveFields` first and truncated afterwards; a truncated body is
replaced by its serialized prefix followed by `...[TRUNCATED <n> chars]`.
