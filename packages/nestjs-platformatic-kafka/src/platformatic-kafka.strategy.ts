  ...
  implements CustomTransportStrategy
{
  ...
  private logPartitionAssignments(data: ConsumerGroupJoinPayload): void {
    ...
    const totalPartitions = assignments.reduce((sum, a) => sum + a.partitions.length, 0);
  ...
  private async attachMessageStream(): Promise<void> {
    ...
    const hasBidirectionalTopics = Array.from(this.messageHandlers.values()).some(
  ...
  public async handleMessage(payload: PlatformaticKafkaMessage): Promise<void> {
    ...
    if (handler?.isEventHandler || !correlationId || !replyTopic) {
      const data = JSON.parse(payload.value) as unknown;
      await super.handleEvent(payload.topic, { pattern: payload.topic, data }, kafkaContext);
      return;
    }
    ...
    this.onProcessingStartHook(this.transportId, kafkaContext, async () => {
      try {
