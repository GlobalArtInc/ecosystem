...
import { MicroserviceOptions } from './microservice.schema';

export function main(options: MicroserviceOptions): Rule {
...
function transform(options: MicroserviceOptions): MicroserviceOptions {
  const target: MicroserviceOptions = Object.assign({}, options);
...
function generate(options: MicroserviceOptions, path: string): Source {
  return apply(url(join('./files' as Path, options?.language || 'ts')), [
    options.spec
      ? noop()
