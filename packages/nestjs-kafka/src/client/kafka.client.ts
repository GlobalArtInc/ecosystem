...
import { formatError, runWithBackoff, sleepMs } from "../utils/kafka-reconnect";
import {
  KafkaConsumer,
  KafkaProducer,
  KafkaMessage,
...
> {
  ...
  private currentStatus: KafkaStatus = KafkaStatus.DISCONNECTED;
  ...
  private async attachResponseStream(): Promise<void> {
    ...
    stream.on("data", (message: KafkaMessage) => {
  ...
  protected deserialize(message: KafkaMessage): IncomingResponse {
